...
sample_df <- get(load('./data/DailyValues.rda'))
sample_data <- data.frame(sample_df)
...
bundled_result <- function(){
  ...
  array <- NULL
  for (i in 1:30){
    today <- damcategory(sample_data[i,1],sample_data[i,2],"Sungai Galas, Dabong",sample_data[i,3],sample_data[i,4])
    tomorrow <- damcategory(sample_data[i+1,1],sample_data[i+1,2],"Sungai Galas, Dabong",sample_data[i+1,3],sample_data[i+1,4])
    week1 <- damcategory(sample_data[i+7,1],sample_data[i+7,2],"Sungai Galas, Dabong",sample_data[i+7,3],sample_data[i+7,4])
    week2 <- damcategory(sample_data[i+14,1],sample_data[i+14,2],"Sungai Galas, Dabong",sample_data[i+14,3],sample_data[i+14,4])
    month <- damcategory(sample_data[i+30,1],sample_data[i+30,2],"Sungai Galas, Dabong",sample_data[i+30,3],sample_data[i+30,4])
